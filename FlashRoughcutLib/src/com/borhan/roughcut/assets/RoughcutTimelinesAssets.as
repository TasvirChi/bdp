...
@ignore
*/
package com.borhan.roughcut.assets
{
	...
	import com.borhan.net.streaming.StreamMetaData;
	...
	[Event(name="roughcutDirty", type="com.borhan.roughcut.events.RoughcutChangeEvent")]
	...
	public class RoughcutTimelinesAssets extends EventDispatcher implements IDisposable
	{
		...
		* all the timelines sorted and organized in one.
		 * @see com.borhan.roughcut.assets.RoughcutTimelinesAssets#buildRoughcutAssets
		*/
		...
		*/
		 ...
		 * @see com.borhan.plugin.logic.Plugin
					...
					if (types & TimelineTypes.TRANSITIONS)
					{
						if (asset.transitionPluginID != TransitionTypes.NONE)
						{
							var transitionAsset:AbstractAsset = AssetsFactory.create (MediaTypes.TRANSITION, asset.assetUID, '0',
												asset.entryName + ".AssetTransition", BorhanApplication.getInstance().getTransitionThumbnail(asset.transitionPluginID), asset.transitionPluginID,
		 ...
		 * @see com.borhan.managers.downloadManagers.protocols.loaders.events.MediaSourceLoaderEvent
		 */
		...
		public function addAssetTransition (transition_type:String, transition_duration:Number, asset_index:uint, timeline:uint = 0x2, cross:Boolean = false):void
		{
			if (timeline & TimelineTypes.VIDEO)
			{
				...
				asset.transitionLabel = BorhanApplication.getInstance().getTransitionLabel(asset.transitionPluginID);
		 ...
		 */
		public function addPlugin (plugin:BorhanPluginInfo, time_stamp:Number, length_sec:Number, timeline:uint = 0x2):uint
		{
			if (timeline & TimelineTypes.VIDEO)
			{
				var pluginAsset:AbstractAsset;
				pluginAsset = AssetsFactory.create (plugin.mediaType, "null", '0',
										plugin.label, BorhanApplication.getInstance().getPluginThumbnail(plugin.pluginId, plugin.mediaType),
		...
		public function resetStreamingAsset (asset:AbstractAsset):void
		{
			if (asset && (asset.mediaType & (MediaTypes.AUDIO | MediaTypes.VIDEO)))
			{
				...
				var partnerInfo:PartnerInfo = BorhanApplication.getInstance().partnerInfo;
 ...
 */		}
		 ...
		 * @see com.borhan.plugin.types.transitions.transitionPluginIDs
		 */
