...
package fl.core {
	...
	public class UIComponent extends Sprite {
		...
		protected function validate():void {
			...
			delete this.callLaterMethods[draw]; // Borhan patch
		}
		/**
         * @private (protected)
         *
...
//			if (inCallLaterPhase) { return; }  // Borhan patch

			if (!callLaterMethods[fn]) // Borhan patch
			{			
				callLaterMethods[fn] = true;
				if (stage != null) {
	                stage.addEventListener(Event.ENTER_FRAME,callLaterDispatcher,false,0,true); // Borhan patch
		...
		private function callLaterDispatcher(event:Event):void 
		{
			if (event.type == Event.ADDED_TO_STAGE) {
                ...
                event.target.removeEventListener(Event.ENTER_FRAME,callLaterDispatcher); // Borhan patch
		    ...
		    callLaterMethods = new Dictionary(); // Borhan patch
